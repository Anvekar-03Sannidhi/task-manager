...
class Task(models.Model): #Database table for tasks-> connected to admin panel-> created DB structure using migrations
    ...
    deadline = models.DateField(null=True, blank=True) #Ex: 2024-06-30 23:59:00
    created_at = models.DateField(auto_now_add=True) #automatically set to current date and time when task is created, cannot be changed later(set only once at creation)
